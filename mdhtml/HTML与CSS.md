...
16. 局部变量和局部函数
	在函数里定义的变量叫局部变量在函数外定义的变量叫全局变量与此类似，局部函数在函数里定义。
17.函数，方法，对象和类
	当使用JavaScript定义一个函数后，实际上可以得到如下四项：
		函数：就像java的方法一样，这个函数可以被调用；
		对象：定义一个函数时，系统也会创建一个对象，该对象是Function类的实例；
		方法：定义一个函数时，该函数通常都会附加给某个对象，作为该对象的方法；
		类：在定义函数的同时，也得到与函数同名的类；
	函数可作为函数被调用，，函数本身也是一个对象，是Function类的实例。
18. 函数的实例属性和类属性
	由于JavaScript函数不仅仅是一个函数，而且是一个类，该函数还是此类唯一的构造器，只要在调用函数时使用new关键字，就可返回一个Object，这个Object不是函数的返回值，而是函数本身产生的对象，因此在JavaScript中定义的变量不仅有局部变量，还有实例属性和类属性两种。根据函数中声明变量的方式，函数中的变量有3种：
		...
		类属性：在函数中以函数名前缀修饰的变量。
19. 调用函数的3种方式
	直接调用函数是最常见，最普通的方式，这种方式直接以函数附加的对象作为调用者，在函数后括号内传入参数来调用函数；
	以call（）方法调用函数，直接调用函数的方式简单，易用，但这种调用方式不够灵活，有时候调用函数需要动态传入一些参数，所以就有了使用call方法来调用函数。
